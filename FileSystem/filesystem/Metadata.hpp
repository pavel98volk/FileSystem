...
Metadata<k,descriptorLength>::Metadata(IOSystem& io):io(io) {
	...
	descrArrLen = ((k*io.getBlockLength())/4 - ((bitsetCharSize + 3) / 4))/4;
...
template <int k, int descriptorLength>
char Metadata<k, descriptorLength>::getChar(int pos) {
...
int Metadata<k, descriptorLength>::getInt(int pos) {
	return ((getChar(pos * 4) << 24) | (getChar(pos * 4 + 1) << 16) | (getChar(pos * 4 + 2) << 8) | (getChar(pos * 4 + 3)));
}
template <int k, int descriptorLength>
...
FileDescriptor<descriptorLength> Metadata<k, descriptorLength>::getDescriptor(int pos) {
	...
	for (int j = 0; j < (descriptorLength); j++) {
		d.data.at(j) = getInt((bitsetCharSize + 3) / 4 + 4 * pos + j);
	}
	return d;
}
...
void Metadata<k, descriptorLength>::setDescriptor(int pos , FileDescriptor<descriptorLength> data) {
	if ((pos < 0) || (pos >= descrArrLen)) throw std::invalid_argument("invalid ");
	for (int j = 0; j < descriptorLength; j++)
		 setInt((bitsetCharSize + 3) / 4 + 4 * pos + j,data.data.at(j));
}
...
void Metadata<k, descriptorLength>::setDescriptorData(int descriptorPos, int partPos, int value) {
	...
	setInt((bitsetCharSize + 3) / 4 + 4 * descriptorPos +partPos, value);
}
...
std::string Metadata<k, descriptorLength>::toPrettyString() {
	...
	for (int j = 0; j < descrArrLen; j++) {
		s += "|(number " + std::to_string(j) + ") " + std::to_string(getDescriptor(j).data[0]) +" " + std::to_string(getDescriptor(j).data[1])+ " " + std::to_string(getDescriptor(j).data[2]) + " " + std::to_string(getDescriptor(j).data[3]) + " |\n";
	}
	s += "\n Free bytes in the end:\n";
	for (int j = (descrArrLen * 16 + bitsetCharSize); j < (io.getBlockLength()*k); j++) {
		s += "X";
		if (j % 64 == 63) s += "\n";
		else if (j % 4 == 3) s += " ";
